...
{% if grains.get('data_disk_device') == "nvme1n1" %}
{% set partition_name = '/dev/' + grains.get('data_disk_device') + 'p1' %}
{% else %}
{% set partition_name = '/dev/' + grains.get('data_disk_device') + '1' %}
{% endif %}

mirror_partition:
  cmd.run:
    - name: /usr/sbin/parted -s /dev/{{grains.get('data_disk_device')}} mklabel gpt && /usr/sbin/parted -s /dev/{{grains.get('data_disk_device')}} mkpart primary 2048 100% && sleep 1 && /sbin/mkfs.{{fstype}} {{partition_name}}
