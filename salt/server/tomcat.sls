...
tomcat_config:
  file.replace:
    ...
    - repl: 'JAVA_OPTS=" $JAVA_OPTS -Xdebug -Xrunjdwp:transport=dt_socket,address={{ grains.get('hostname') }}.{{ grains.get('domain') }}:8000,server=y,suspend=n "'
    {% else %}
    - repl: 'JAVA_OPTS=" $JAVA_OPTS -Xdebug -Xrunjdwp:transport=dt_socket,address={{ grains.get('fqdn') }}:8000,server=y,suspend=n "'
    {% endif %}
...
extend_tomcat_login_timeout:
  file.replace:
    ...
    - repl: <session-timeout>{{ grains.get('login_timeout') // 60 }}</session-timeout>
    - append_if_not_found: True
    - require:
        - cmd: server_setup
{% endif %}
